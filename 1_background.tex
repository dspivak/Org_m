...
\]\dnote{Sophie, do you prefer to use $\smcat^{\sharp\tn{cart}}/-$ notation or $\smset^-$? The first is more honest, but the second is more familiar. Please choose and fill throughout the document.}

This gives a change of enrichment, which sends $\org^\sharp$ to the monoidal 2-category whose objects are polynomials under $\otimes$ and for which $\hom(p,q)=\smcat^{\sharp\tn{cart}}/\cofree_{[p,q]}$. This is equivalent to the category-enriched operad $\org$ defined in \cite[Definition 2.19]{spivak2021learnersv1}.
